...
studentRoutes.get("/", async (c) => {
  ...
  const [mySessions, myLaps, publicLaps, progress] = await Promise.all([
    getSessionsForUser(user.id),
    getReferenceLapsForUser(user.id),
    getPublicReferenceLaps(),
    getTrackProgress(user.id),
  ]);
  return c.html(studentDashboardPage(user, mySessions, myLaps, publicLaps, progress));
});